...
exports.getWebpackConfig = (eggWebpackConfig, option) => {
  ...
  if (!framework) {
    const pkgFilepath = path.join(option.baseDir, 'package.json');
    const pkg = require(pkgFilepath);
    if (pkg.dependencies['egg-view-vue-ssr']) {
      framework = 'vue';
    }
    if (pkg.dependencies['egg-view-react-ssr']) {
      framework = 'react';
    }
  }
  assert(framework, 'please set [egg-webpack] plugin config.framework: vue/react');
